...
export default function UserProfile() {
    ...
    async function handleSubmitProfile(e) {
        ...
        console.log(responseBody);
        if (response.status === 200) {
            await alertSuccess("Profile update succesfully");
        } else {
            await alertError(responseBody.errors);
        }
    ...
    async function handleSubmitPassword(e) {
        ...
        if (password !== confirmPassword) {
            await alertError("Password and confirm password are not match");
            return;
        }
        ...
        if (response.status === 200) {
            setPassword('');
            setConfirmPassword('');
            await alertSuccess("Password update succesfully");
        } else {
            await alertError(responseBody.errors);
        }
    ...
    useEffectOnce(() => {
        fetchUserDetail()
            .then(() => console.log("user detail fatched succesfully"));
    });
